...
// blocked or not
bool isUnBlocked(char grid[ROW][COL], int row, int col)
{
...
// to destination
void tracePath(cell cellDetails[ROW][COL],Pair midpoint,char grid[ROW][COL])
{
    cout<<"\nThe Path is "<<endl;

    int rowfor = midpoint.first;
    int colfor = midpoint.second;
    int rowbac = midpoint.first;
    int colbac = midpoint.second;
   // cout<< row <<" "<<col<<endl;
    stack<Pair> Pathfor;
    stack<Pair> Pathbac;
    // for(int i = 0; i < ROW; i++)
...
void expandForward(set<pPair>& openList,cell cellDetails[ROW][COL],bool closedList[ROW][COL],char grid[ROW][COL],double& UB, Pair start, Pair dest,double& optimalcost,Pair& endpoint)
{
...
void expandBackward(set<pPair>& openList,cell cellDetails[ROW][COL],bool closedList[ROW][COL],char grid[ROW][COL],double& UB, Pair start, Pair dest,double& optimalcost,Pair& endpoint)
{
...
void dibbSearch(char grid[ROW][COL], Pair start, Pair dest)
{
   ...
   /*
    ...
    while(UB>((f1min+f2min)/2.0))
    {
        if(d == 1)
        {
            expandForward(openList_1,cellDetails,closedList,grid,UB,start,dest,optimalcost,endpoint);
            d++;
            pPair p1 = *openList_1.begin();
            f1min = p1.first;
            if(f1min == 0)
             {   f1min = FLT_MAX; cout<<"Forward expansion ended."<<endl;}
            // lastforward = p1.second;
            // if(//UB!=FLT_MAX&&
            // (cellDetails[p1.second.first][p1.second.second].g_1+cellDetails[p1.second.first][p1.second.second].g_2)<optimalcost)
            // {  
            //     optimalcost = cellDetails[p1.second.first][p1.second.second].g_1+cellDetails[p1.second.first][p1.second.second].g_2;
            //     endpoint = p1.second;
            //     cout<<p1.second.first<<", "<<p1.second.second<<endl;
            //     output<<"current expand node is "<<p1.second.first<<" , "<<p1.second.second<<endl;
            // }
            
        }    
        else
        {
            expandBackward(openList_2,cellDetails,closedList,grid,UB,start,dest,optimalcost,endpoint);
            d--;
            pPair p2 = *openList_2.begin();
            f2min = p2.first;
            if(f2min==0)
            {    f2min=FLT_MAX; cout<<"Backward expansion ended."<<endl;}
            // lastbackward = p2.second;
            // if(//UB!=FLT_MAX&&
            //     (cellDetails[p2.second.first][p2.second.second].g_1+cellDetails[p2.second.first][p2.second.second].g_2)<optimalcost)
            // {
            //     optimalcost = cellDetails[p2.second.first][p2.second.second].g_1+cellDetails[p2.second.first][p2.second.second].g_2;
            //     endpoint = p2.second;
            //     cout<<p2.second.first<<", "<<p2.second.second<<endl;
            //     output<<"current expand node is "<<p2.second.first<<" , "<<p2.second.second<<endl;
            // }
        }
       // cout<< "UB: " << UB << endl; 
    ...
    tracePath(cellDetails,endpoint,grid);
    //printmap(grid);
    return;
